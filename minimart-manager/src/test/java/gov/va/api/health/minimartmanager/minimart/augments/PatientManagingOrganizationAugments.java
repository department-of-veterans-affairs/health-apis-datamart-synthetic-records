...
public class PatientManagingOrganizationAugments {
    ...
    private static final List<Optional<String>> ORGANIZATION_REFERENCES =
            loadOrganizations();

    static DatamartPatient addManagingOrganization(Augmentation.Context<DatamartPatient> ctx){
    return ctx.resource()
        .managingOrganization(ctx.random(ORGANIZATION_REFERENCES));
    }
        ...
        ReferenceLoader.loadReferencesFor(
            ...
            dm ->
                DatamartReference.builder()
                    ...
                    .build()).stream().map(d -> d.flatMap(DatamartReference::reference)).collect(Collectors.toList());
